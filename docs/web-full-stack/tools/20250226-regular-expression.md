...
假设有字符串 `"2025-02-26"`，想提取年份、月份和日期，可以使用捕获组：
...
-   第一个捕获组 `(\d{4})` 会匹配年份 `"2025"`，
-   第二个捕获组 `(\d{2})` 会匹配月份 `"02"`，
-   第三个捕获组 `(\d{2})` 会匹配日期 `"26"`。

如果只是想验证格式而不需要提取各部分，可以使用非捕获组：
...
反向引用（Backreference）是一种正则表达式的功能，用于**引用之前捕获组所匹配的内容**。它的主要作用是确保匹配的文本在不同位置保持一致。

常见应用场景包括检测重复内容、匹配成对的符号或验证输入格式的对称性。
